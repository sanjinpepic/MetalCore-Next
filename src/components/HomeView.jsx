    ...
    return (
        <div className="flex flex-col flex-1 min-w-0 h-screen overflow-y-auto custom-scrollbar bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black">
            ...
            <div className="p-6 md:p-16 pt-24 md:pt-24 space-y-4 md:space-y-6 max-w-5xl">
                ...
                <h1 className="text-5xl md:text-8xl font-display font-black text-white tracking-tighter italic leading-[0.9] md:leading-none">
                    ADVANCED<br />
                ...
                <p className="text-slate-400 text-base md:text-2xl max-w-2xl leading-relaxed">
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                    <button
                        ...
                        className="px-6 md:px-8 py-4 md:py-5 bg-white text-black rounded-xl font-bold hover:bg-slate-200 transition-all flex items-center justify-center gap-2 group text-base md:text-lg"
                    >
                        Browse Library
                    ...
                    <button
                        ...
                        className="px-6 md:px-8 py-4 md:py-5 bg-white/5 text-white border border-white/10 rounded-xl font-bold hover:bg-white/10 transition-all text-base md:text-lg"
                    >
                        View Matrix
                    </button>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 px-6 md:px-16 mb-12 md:mb-16 max-w-6xl">
                {stats.map((stat, i) => (
                    <div key={i} className="glass-panel p-6 rounded-2xl border-white/5">
                        ...
                        <div className="text-base font-bold text-slate-500 uppercase tracking-widest">{stat.label}</div>
                    </div>
                ))}
                <div className="glass-panel p-6 rounded-2xl border-white/5 bg-gradient-to-br from-accent/20 to-transparent border-accent/20">
                    ...
                    <div className="text-base font-bold text-accent/80 uppercase tracking-widest">Version</div>
                </div>
            </div>
                ...
                <div className="px-6 md:px-16 pb-16">
                    ...
                    <div className="glass-panel p-8 md:p-12 rounded-[2rem] md:rounded-[2.5rem] border-white/5 flex flex-col lg:flex-row gap-8 md:gap-12 group hover:border-white/10 transition-all relative overflow-hidden">
                        <div className="lg:w-1/2 relative z-10">
                            <div className="text-accent font-bold uppercase tracking-widest text-base mb-2">{featuredSteel.producer}</div>
                            <h2 className="text-4xl md:text-6xl font-black text-white italic mb-4 md:mb-6">{featuredSteel.name}</h2>
                            <p className="text-slate-400 text-base md:text-xl mb-6 md:mb-8 leading-relaxed italic">"{featuredSteel.desc}"</p>

                            <button
                                onClick={() => setDetailSteel(featuredSteel)}
                                className="inline-flex items-center gap-2 text-base font-bold text-white border-b border-accent pb-1 hover:text-accent transition-colors"
                            >
                                View Data Sheet
                        ...
                        <div className="lg:w-1/2 grid grid-cols-2 gap-4 relative z-10">
                            {['edge', 'toughness', 'corrosion', 'sharpen'].map(metric => (
                                <div key={metric} className="bg-black/40 rounded-xl md:rounded-2xl p-5 border border-white/5">
                                    <div className="text-base text-slate-500 uppercase font-black mb-2">{metric}</div>
                                    ...
                                    <div className="text-right text-base font-mono font-bold text-white mt-2">{featuredSteel[metric]}/10</div>
                                </div>
                            ))}
                        </div>
